...
import 'prosedur_data.dart';

class ProsedurPerjalananPage extends StatefulWidget {
  const ProsedurPerjalananPage({super.key});

  @override
  State<ProsedurPerjalananPage> createState() => _ProsedurPerjalananPageState();
}

class _ProsedurPerjalananPageState extends State<ProsedurPerjalananPage>
    with SingleTickerProviderStateMixin {
  late final TabController _tab;
  String _q = '';
  Set<String> _favs = {};            // key: sectionId|index
  Map<String, bool> _checks = {};    // checklist selections

  @override
  void initState() {
    super.initState();
    _tab = TabController(length: prosedurSections.length + 1, vsync: this); // +1 tab Checklist
    _loadState();
  }

  Future<void> _loadState() async {
    final sp = await SharedPreferences.getInstance();
    _favs = (sp.getStringList('travel_proc_favs') ?? []).toSet();
    final raw = sp.getStringList('travel_proc_checks') ?? [];
  ...
  Future<void> _toggleFav(String key) async {
    ...
    await sp.setStringList('travel_proc_favs', _favs.toList());
    if (mounted) setState(() {});
  }
  ...
  Future<void> _toggleCheck(String item) async {
    ...
    await sp.setStringList('travel_proc_checks', list);
    if (mounted) setState(() {});
  }
  ...
  Widget build(BuildContext context) {
    final tabs = [
      ...prosedurSections.map((s) => Tab(text: s.title)),
    ...
    return Scaffold(
      ...
      body: Container(
        ...
        child: Column(
          children: [
            if (_tab.index < prosedurSections.length)
              ...
              Container(
                ...
                child: TextField(
                  decoration: InputDecoration(
                    hintText: 'Cari di "${prosedurSections[_tab.index].title}"…',
            ...
            Expanded(
              child: TabBarView(
                controller: _tab,
                children: [
                  ...prosedurSections.map((s) => _SectionView(
                        section: s,
                        query: _q,
                        favorites: _favs,
  ...
  Widget build(BuildContext context) {
    final entries = query.trim().isEmpty
        ...
        : section.entries.where((e) =>
            ...
            e.bullets.any((b) => b.toLowerCase().contains(query.toLowerCase()))).toList();
                        ...
                        children: [
                          ...
                          Container(
                            ...
                            child: IconButton(
                              tooltip: 'Salin',
                              onPressed: () async {
                                final text = [e.title, ...e.bullets.map((b) => '• $b')].join('\n');
...
class _ChecklistView extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final items = {
      ...{for (final s in prosedurChecklist) s: true},
      ...checks,
    }.keys.toList();
